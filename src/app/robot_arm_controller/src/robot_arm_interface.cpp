#include "robot_arm_controller/robot_arm_interface.h"
#include <std_msgs/UInt16MultiArray.h>
#include "robot_arm_controller/AnglesConverter.h"


RobotArmInterface::RobotArmInterface(ros::NodeHandle& nh) : nh_(nh), 
            pnh_("~"),
            pos_(4, 0),
            vel_(4, 0),
            eff_(4, 0),
            ...
            names_{"joint_1", "joint_2", "joint_3", "joint_4"}
{
    ...
    hardware_pub_ = pnh_.advertise<std_msgs::UInt16MultiArray>("/robot_arm/arm_actuate", 1000);
    hardware_srv_ = pnh_.serviceClient<robot_arm_controller::AnglesConverter>("/radians_to_degrees");
    
    ROS_INFO("Starting Robot Arm Hardware Interface...");
    ...
    looper_ = nh_.createTimer(update_freq_, &RobotArmInterface::update, this);
    
    ROS_INFO("Ready to execute the control loop");
}

void RobotArmInterface::update(const ros::TimerEvent& e)
{
...
void RobotArmInterface::write(ros::Duration elapsed_time)
{    
    ...
    robot_arm_controller::AnglesConverter srv;
...
{
    ros::init(argc, argv, "robot_arm_interface_node");
    ros::NodeHandle nh;
    ros::MultiThreadedSpinner spinner(2);
    RobotArmInterface robot(nh);
